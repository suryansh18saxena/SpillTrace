...
| P23-017 | A2 | Plain-language layer: one explanation dictionary, per-screen "What am I looking at?" strip, inline `?` tips, and a per-case Walkthrough narrating the chain with its real figures | P23-007 | `frontend/src/lib/explain.ts`, `components/explain/`, `app/(app)/cases/[caseId]/walkthrough/`, `components/layout/PageHeader.tsx` | 11 new vocabulary/lookup tests (165 total green); guide verified on 5 screens, collapse persists across navigation and reload; walkthrough renders 8 steps from live case data with 0 console errors at 1440px and 400px | COMPLETED |
| P23-018 | A2 | First-run tour explains the product before the navigation, and the pipeline list shows everyday stage names beside the machine names | P23-004 | `components/shell/OnboardingTour.tsx`, `components/jobs/JobProgress.tsx` | Tour opens on "What this system is for" and closes on the responsibility caveat; `PREF_TOUR_DONE` bumped to v2 so returning users see it once | COMPLETED |
| P23-019 | A2 | Written user guide: every screen, how to register a case, analyst vs administrator, a five-minute demo script, troubleshooting and a glossary | P23-017 | `docs/user-guide.html` → `SPILLTRACE_User_Guide.pdf` | 19 pages A4; every screen surveyed from the running app, not from source; banned vocabulary appears only inside negations | COMPLETED |
| P23-020 | A2 | New case: one-click time windows and a UTC echo, because the native `datetime-local` renders in the browser's locale order and only opens from a small glyph | P23-019 | `app/(app)/cases/new/page.tsx`, `styles/pages.module.css`, `components/ui/ui.module.css` | Presets fill both fields (verified in headless Chrome); hint echoes the resolved window and duration; picker glyph enlarged with a hover state; user guide regenerated | COMPLETED |
